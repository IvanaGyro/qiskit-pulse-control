...
class RabiScanAmplitudes(meta.QiskitTask):
    ...
    def build_circuit(self, backend, duration, amplitude, sigma):
        with pulse.build(backend) as gate_pulse:
            microwave = Gaussian(duration, amplitude, sigma)
            pulse.play(microwave, pulse.drive_channel(0))
    ...
    def submit_job(self):
        ...
        # duration must be a multiple of 16 cycles
        # duration should be at least 64dt and must be int
        duration = int((self.sigma * 6 + 15) // 16 * 16)
        duration = max(duration, 64)
        circuits = [
            self.build_circuit(backend, duration, amplitude, self.sigma)
...
@dataclass
class RabiGaussian(meta.QiskitTask):
    amplitudes: list[float]
    sigma: float
    duration: int
    physical_qubit: int
    ...
    def submit_job(self):
        ...
        if self.duration % 16 != 0:
            raise ValueError('duration must be a multiple of 16 cycles')
        if self.duration < 64:
            raise ValueError('duration should be at least 64dt')
        circuits = [
            self.build_circuit(backend, self.duration, amplitude, self.sigma)
            for amplitude in self.amplitudes
        ]
        sampler = SamplerV2(mode=backend)
        job = sampler.run(circuits)
    ...
    def post_process(self, result):
        ...
        plt.scatter(amplitudes, proportions, color='blue', label='Data Points')
        plt.plot(x_fit, y_fit, color='red', label='Fitted Curve')
        plt.title(rf'Gaussian Pulse, $\sigma = {self.sigma}$ duration = {self.duration}')
        plt.xlabel('Amplitude')
        plt.ylabel(r'Proportion of $|1\rangle$')
...
def main():
    ...
    #     'ibm_sherbrooke', amplitudes=[1], sigmas=list(range(5, 101))).run()
    ...
    # TODO: replace `RabiScanAmplitudes` with `RabiGaussian`
    # RabiScanAmplitudes(
    #     'ibm_sherbrooke',
    ...
    RabiGaussian(
        'ibm_sherbrooke',
        amplitudes=[0.02 * i for i in range(1, 39)],
        sigma=64,
        duration=256,
        physical_qubit=0).run()
